...
const getOverdueMonths = (payments, dueDate) => {
    ...
    for (let month = 1; month <= currentMonth; month++) {
        ...
        if (!(payments || []).some(p => p.month === monthStr) && (month < currentMonth || (month === currentMonth && currentDate > dueDate))) {
...
const getOverpaidMonths = (payments) => {
    ...
    (payments || []).forEach(p => {
        ...
        if (year > currentYear || (year === currentYear && month > currentMonth)) {
            overpaidMonths.push(p.month);
        }
    });
    return overpaidMonths;
};
...
const getPaymentStatus = (property) => {
    if (getOverdueMonths(property.payments || [], property.dueDate || 1).length > 0) return { text: t('overdue'), color: '#EF4444' };
    if (getOverpaidMonths(property.payments || []).length > 0) return { text: t('overpaid'), color: '#10B981' };
    const currentMonthStr = `${new Date().getFullYear()}-${String(new Date().getMonth() + 1).padStart(2, '0')}`;
    if ((property.payments || []).some(p => p.month === currentMonthStr)) return { text: t('paid'), color: Colors.light.primary };
    return { text: t('pending'), color: '#F59E0B' };
};
...
export default function GroupDetailScreen() {
        ...
        try {
            ...
            if (currentGroup && currentGroup.properties) {
                ...
                currentGroup.properties.forEach(p => {
                    if (getOverdueMonths(p.payments, p.dueDate).length > 0) due.push(p);
                    else if (getOverpaidMonths(p.payments).length > 0) overpaid.push(p);
                    else paid.push(p);
                });
                const sectionData = [
    ...
    const handleSaveProperty = (property) => {
        ...
        const updatedProperties = selectedProperty
          ? currentGroup.properties.map(p => (p.id === property.id ? property : p))
          : [...currentGroup.properties, property];
        
        saveGroup({ ...currentGroup, properties: updatedProperties });
        setIsPropertyModalVisible(false);
      };
    
      const handleDeleteProperty = (propertyId) => {
        const currentGroup = { ...group };
        const updatedProperties = currentGroup.properties.filter(p => p.id !== propertyId);
        saveGroup({ ...currentGroup, properties: updatedProperties });
        setIsPropertyModalVisible(false);
      };
      ...
      const handleSavePayments = (newPayments) => {
        ...
        const currentGroup = { ...group };
        const updatedProperties = currentGroup.properties.map(p => 
        ...
        return (
            <View style={[styles.propertyCard, { backgroundColor: cardColor }]}>
                <View style={styles.propertyHeader}>
                    ...
                    <TouchableOpacity onPress={() => openPropertyModal(item)} style={styles.editButton}>
